...
if (count($arrError) > 0) {
    ...
    $sql = " SELECT tot.*, tu.user_name as customer_name, tc.course_time FROM tbl_orders tot inner join tbl_user"
            ...
            . " inner join tbl_course tc on tc.id = tot.course_id where tot.user_id =  "
            . $_GET['trainee_id']." and tot.status not in ('Under Process','Pending')";
    //echo $sql;
    ...
    if ($db->affected_rows > 0) {
        ...
        if (!empty($arrTmp)) {
            ...
            $arrRecord['data']['success'] = 0;
            $arrRecord['data']['view_order'] = 'Either  record is not found or not approved from admin';
        }
    } else {
        $arrRecord['data']['success'] = 0;
        $arrRecord['data']['view_order'] = 'Either  record is not found or not approved from admin';
