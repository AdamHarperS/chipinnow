...
$querya=mysqli_query($conn,"select * from firebase_keys");
$resa=mysqli_fetch_array($querya);
$google_api_key=$resa['androidkey'];

// Notification Data
$query=mysqli_query($conn,"select * from notification_users where device='android'");
$i=0;
$reg_id=array();
...
$massage = $_POST['message1'];
        ...
        if ($result === FALSE){
            ...
            if($response['success']>0)
            {
                ...
                $notification="insert into tbl_notifications values(NULL,'".$massage."','".$date."')";
...
$queryi=mysqli_query($conn,"select * from firebase_keys");
$resi=mysqli_fetch_array($queryi);
$ios_api_key=$resa['ioskey'];

$queryios=mysqli_query($conn,"select * from notification_users where device='ios'");
$i=0;
$reg_id=array();
